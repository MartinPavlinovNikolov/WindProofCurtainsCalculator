<table style="position: relative;" class="table table-condensed table-sm table-bordered text-center">
	...
	<tbody>
		<tr v-for="(order, o) in orders" :class="isCompleated(o) ? 'table-success' : 'd-none'">
	    	...
	    	<td>
				...
				<button type="button" class="btn btn-sm btn-primary" data-toggle="modal" :data-target="'#exampleModalCenterC'+order.id+'A'">виж</button>
					...
					<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
						<div class="modal-content">
							...
							<div class="modal-body" style="background-color: #F0F0F0;">
								...
								<div class="row mt-3">
									<div class="col-12">
										<ul class="nav nav-tabs" id="myWallTabC" role="tablist">
										  	<li v-for="(wall, w) in order.walls" class="nav-item">
										    	<a class="nav-link" :class="w == 0 ? 'active' : ''" :id="'wall-tabC'+wall.id+order.id" data-toggle="tab" :href="'#wallC'+wall.id+order.id" role="tab" :aria-controls="'wallC'+wall.id+order.id" :aria-selected="w == 0 ? 'true' : 'false'">{{(w + 1)}}</a>
										  	</li>
										</ul>
									</div>
